...
- A segmented **mode** picker (Standard / Dictation) that selects the `/correct` profile — `default` or `dictation` server-side. Standard handles punctuation, casing, light cleanup, and normalization of complete emails, phone numbers, URLs, IDs, and versions; Dictation layers in spoken punctuation and line/paragraph commands. The mode can be changed anytime.
