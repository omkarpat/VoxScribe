...
- An editable **keyterms** list (Standard transcriber only): add, rename (tap a row), and swipe-to-delete. Terms bias the transcriber and are sent as `protected_terms` to `/correct`. Keyterm edits are disabled while recording — stop the session first; changes take effect on the next session. The list is persisted in `UserDefaults` and seeded with a small Hinglish starter set (bhai, theek hai, chai, haan, na) on first launch.
