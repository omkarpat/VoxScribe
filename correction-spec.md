...
- Normalize complete structured tokens (email, phone, URL, version, numeric ID) within `default` when every required component is present in the raw turn.
...
Profiles:
- `default`: punctuation, casing, filler cleanup, protected-term preservation, and normalization of complete structured tokens (emails, phone numbers, URLs, versions, numeric IDs) when every required component is present in the raw turn.
- `dictation`: `default` plus spoken punctuation and line/paragraph commands.

The previously separate `structured_entry` profile has been retired. Its normalization behavior now lives in `default`; its partial-field safety rules are enforced by the existing specificity-discipline rules. The server accepts `structured_entry` as a deprecated alias of `default` for one compatibility window.
...
- Normalization of complete structured tokens (emails, phone numbers, URLs, versions, numeric IDs) in `default` when every required component is present.
...
- Structured-text normalization for emails, phone numbers, URLs, IDs, and versions under `default`, including partial-field safety cases.
