...
- Add exactly two server-minted templates:
  - **standard** — natural conversational formatting with conservative support for common structured text,
  - **dictation** — standard behavior plus spoken punctuation / paragraph commands.
- Templates live server-side (`server/providers/assemblyai.py`) and are selected by a `prompt_template` field on `POST /token`.
- No free-form prompt editor in the app — the surface is strictly the two templates. Do not reintroduce a separate structured-entry template; that behavior belongs in Standard when strongly supported.
...
- Triggers:
  ...
  - prompt template changes (e.g. Standard ↔ Dictation in §5),
...
- When ON:
  - select the dictation streaming prompt template (§3),
...
- **Two prompt templates are enough.** Resist prompt sprawl. Phase 4 adds exactly two streaming templates, Standard and Dictation; a separate structured-entry prompt and free-form prompt UI are explicitly out of scope.
...
- Prompt-template switch (Standard → Dictation) works at session start and mid-session without reconnecting.
